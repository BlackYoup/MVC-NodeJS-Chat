module.exports = function(_config){
	...
	sessionSockets.on('connection', function(err, socket, session){
		...
		socket.on('login', function(usrPseudo){
			var reponse = funcs.login(usrPseudo, session);
		...
		socket.on('newMessage', function(message){
			funcs.addMessage(message, session.room, message.from || session.userName);
		});
		...
		socket.on('multiPing', function(users){
			funcs.multiPing(users);
		});

		socket.on('disconnect', function(){
			Debug('disconnecting ' + session.userName);
