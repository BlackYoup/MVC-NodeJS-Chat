...
var BAN_TIME = 3600;
var SAVE_MESSAGES = false;

if(SAVE_MESSAGES){
    var connectPostgre = "postgres://developpement:developpement@127.0.0.1/nodejsChat";
    var postgreApi = require('pg');
}
...
exports.reassignRooms = function(socket, sess){
    var session = sess || {};
...
exports.addMessage = function(message, session, pseudo, isFromBDD){
    ...
    if(message.text === ''){
        ...
        if(isFromBDD || checkUserSpam(session.room, pseudo)){
            io.sockets.in(session.room).emit('message', {text: message.text, from: pseudo, room: session.room, time: message.time || null});
            
            if(!isFromBDD){
                Debug('wanna save');
                saveMessage(session.room, pseudo, message.text);
            }
        }
    }    
};
...
exports.login = function(usrPseudo, session, ip){
    ...
    for(var userPseudo in allUsers){
...
exports.setLeft = function(userName, time){
    time = (time === null ? null : new Date().getTime());
...
exports.sendLastMessages = function(session){
    if(!SAVE_MESSAGES){
        return false;
    }

    var postgre = newPostGreConnexion();

    postgre.connect(function(err){
        if(err) throw err;

        postgre.query('SELECT author, message, timestamp, room FROM messages WHERE room = $1 ORDER BY idmessage DESC LIMIT 10 OFFSET 0', [session.room], function(err, results){
            if(err) throw err;

            for(var i = results.rows.length, j = 0; i > j; i--){
                sendPersonnalMessage(allUsers[session.userName].socket.id, {text: results.rows[i - 1].message, time: results.rows[i - 1].timestamp}, {room: results.rows[i - 1].room}, results.rows[i - 1].author.trim(), true);
            }
...
function saveMessage(room, author, message){
    if(!SAVE_MESSAGES){
        return false;
    }
    Debug('saving');
    ...
    postgre.connect(function(err){
        ...
        postgre.query("INSERT INTO messages (author, message, timestamp, room) VALUES($1, $2, $3, $4)", [author, message, new Date().getTime(), room], function(err){
            if(err) throw err;
            postgre.end();
        });
    });
...
function newPostGreConnexion(){
    return new postgreApi.Client(connectPostgre);
}

function sendPersonnalMessage(socketID, message, session, pseudo){
    io.sockets.socket(socketID).emit('message', {text: message.text, from: pseudo, room: session.room, time: message.time || null});
}

setInterval(function(){
    var date = new Date().getTime() - 30000;
    for(var usrPseudo in allUsers){
...
// liste de promise(A) en promise de liste(A)